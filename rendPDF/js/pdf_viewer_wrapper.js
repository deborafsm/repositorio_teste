import{html,PolymerElement,dom,Polymer,Base}from"chrome://resources/polymer/v3_0/polymer/polymer_bundled.min.js";import{isIOS,isWindows,isMac}from"chrome://resources/js/cr.m.js";import{ZoomBehavior}from"chrome-extension://mhjfbmdgcfjbbpaeojofohoefgiehjai/browser_api.js";export{BrowserApi,ZoomBehavior}from"chrome-extension://mhjfbmdgcfjbbpaeojofohoefgiehjai/browser_api.js";import{loadTimeData}from"chrome://resources/js/load_time_data.m.js";
/**
@license
...
const FittingType={NONE:"none",FIT_TO_PAGE:"fit-to-page",FIT_TO_WIDTH:"fit-to-width",FIT_TO_HEIGHT:"fit-to-height"};const SaveRequestType={ANNOTATION:0,ORIGINAL:1,EDITED:2};// Copyright (c) 2010 The Chromium Authors. All rights reserved.
// Use of this source code is governed by a BSD-style license that can be
// found in the LICENSE file.
...
`;document.head.appendChild(template$2.content);// Copyright 2020 The Chromium Authors. All rights reserved.
const template$3=document.createElement("template");template$3.innerHTML=`<dom-module id="pdf-shared">\x3c!--_html_template_start_--\x3e\n<template>\n  <style scope="pdf-shared">cr-icon-button {\n  --cr-icon-button-fill-color: var(--pdf-toolbar-text-color);\n      --cr-icon-button-focus-outline-color: var(--google-grey-500);\n      margin: 0;\n}\n\ncr-icon-button:hover {\n  background: rgba(255, 255, 255, 0.08);\n      border-radius: 50%;\n}\n\ncr-action-menu, viewer-bookmark {\n  --cr-menu-background-color: var(--google-grey-900);\n      --cr-menu-shadow: rgba(0, 0, 0, .3) 0 1px 2px 0,\n                        rgba(0, 0, 0, .15) 0 3px 6px 2px;\n      --cr-primary-text-color: var(--google-grey-200);\n      --cr-menu-background-focus-color: var(--google-grey-refresh-700);\n      --cr-menu-background-sheen: rgba(255, 255, 255, .06);\n      --cr-separator-line: var(--cr-separator-height) solid\n          rgba(255, 255, 255, .1);\n}\n\n</style>\n</template>\n\x3c!--_html_template_end_--\x3e</dom-module>\n`;document.body.appendChild(template$3.content.cloneNode(true));// Copyright 2015 The Chromium Authors. All rights reserved.
  ...
  transform: rotate(90deg);
    ...
    <div id="item" on-click="onClick_">
      <div id="expand-container">
        <cr-icon-button id="expand" iron-icon="cr:chevron-right" aria-label="Secção" aria-expanded$="[[getAriaExpanded_(childrenShown_)]]" on-click="toggleChildren_"></cr-icon-button>
      </div>
...
<!--_html_template_end_-->`;document.head.appendChild(template$4.content);// Copyright 2020 The Chromium Authors. All rights reserved.
...
<cr-icon-button id="download" iron-icon="cr:file-download" on-click="onDownloadClick_" aria-label="Transferir" aria-haspopup$="[[downloadHasPopup_]]" title="Transferir"></cr-icon-button>
...
#divider {
  margin: 0 var(--page-selector-spacing);
    ...
    <div id="content">
      <input part="input" type="text" id="pageselector" value="[[pageNo]]" on-pointerup="select" on-input="onInput_" on-change="pageNoCommitted" aria-label="Número de página">
...
class ViewerPasswordDialogElement extends PolymerElement{static get is(){return"viewer-password-dialog"}static get template(){return html`<!--css-build:shadow--><!--_html_template_start_--><style include="cr-shared-style" scope="viewer-password-dialog">#password {
  margin-top: var(--cr-form-field-bottom-spacing);
    ...
    <cr-dialog id="dialog" no-cancel="" show-on-attach="">
      <div slot="title">É necessária a palavra-passe</div>
      <div slot="body">
        <div id="message">Este documento está protegido por palavra-passe.  Introduza uma palavra-passe.</div>
        <cr-input id="password" type="password" error-message="Palavra-passe incorrecta" invalid="[[invalid]]" autofocus="">
        </cr-input>
      </div>
...
<div id="icons" hidden$="[[!bookmarks.length]]">
  <div class$="
    ...
    <cr-icon-button iron-icon="pdf:thumbnails" role="tab" title="Miniaturas" aria-label="Miniaturas" aria-selected$="[[getAriaSelectedThumbnails_(thumbnailView_)]]" on-click="onThumbnailClick_">
    </cr-icon-button>
  </div>
  <div class$="
      button-wrapper cr-vertical-tab [[outlineButtonClass_(thumbnailView_)]]">
    <cr-icon-button iron-icon="pdf:doc-outline" role="tab" title="Contorno do documento" aria-label="Contorno do documento" aria-selected$="[[getAriaSelectedOutline_(thumbnailView_)]]" on-click="onOutlineClick_">
    </cr-icon-button>
  </div>
</div>
<div id="content">
...
<cr-dialog show-on-attach="">
  <div slot="title">Propriedades do documento</div>
  <div slot="body">
    <table>
      <tbody><tr>
        <td class="name">Nome do ficheiro:</td>
        <td class="value" id="file-name">[[fileName]]</td>
      </tr>
      <tr class="break">
        <td class="name">Tamanho do ficheiro:</td>
        <td class="value" id="file-size">[[documentMetadata.fileSize]]</td>
      </tr>
      <tr>
        <td class="name">Título:</td>
...
<div id="toolbar">
  ...
  <div id="center">
    ...
    <span id="zoom-controls">
      <cr-icon-button iron-icon="pdf:remove" title="Reduzir" disabled="[[isAtMinimumZoom_(zoomBounds.min, viewportZoomPercent_)]]" aria-label="Reduzir" on-click="onZoomOutClick_">
      </cr-icon-button>
      <input type="text" value="100%" aria-label="Nível de zoom" on-change="onZoomChange_" on-pointerup="onZoomInputPointerup_" on-blur="onZoomChange_">
      
      <cr-icon-button iron-icon="pdf:add" title="Ampliar" disabled="[[isAtMaximumZoom_(zoomBounds.max, viewportZoomPercent_)]]" aria-label="Ampliar" on-click="onZoomInClick_">
      </cr-icon-button>
    </span>
    <span class="vertical-separator"></span>
    <cr-icon-button id="fit" iron-icon="[[fitToButtonIcon_]]" title="[[getFitToButtonTooltip_('Ajustar à página',
                                        'Ajustar à largura',
                                        fittingType_)]]" aria-label="[[getFitToButtonTooltip_('Ajustar à página',
                                             'Ajustar à largura',
                                             fittingType_)]]" on-click="onFitToButtonClick_">
    </cr-icon-button>
    <cr-icon-button iron-icon="pdf:rotate-left" dir="ltr" aria-label="Rodar para a esquerda" title="Rodar para a esquerda" on-click="onRotateClick_">
    </cr-icon-button>
  </div>
  <div id="end">
    ...
    <cr-icon-button id="print" iron-icon="cr:print" hidden="[[!printingEnabled]]" title="Imprimir" aria-label="Imprimir" on-click="onPrintClick_">
    </cr-icon-button>
    <cr-icon-button id="more" iron-icon="cr:more-vert" title="Mais ações" aria-label="Mais ações" on-click="onMoreClick_"></cr-icon-button>
  </div>
</div>
...
class ViewerErrorDialogElement extends PolymerElement{static get is(){return"viewer-error-dialog"}static get template(){return html`<!--css-build:shadow--><!--_html_template_start_--><style include="cr-hidden-style" scope="viewer-error-dialog"></style>
    <cr-dialog id="dialog" no-cancel="" show-on-attach="">
      <div slot="title">Erro</div>
      <div slot="body">Falha ao carregar o documento PDF.</div>
...
class LocalStorageProxyImpl{getItem(key){return window.localStorage.getItem(key)}setItem(key,value){window.localStorage.setItem(key,value)}static getInstance(){return instance$1||(instance$1=new LocalStorageProxyImpl)}}let instance$1=null;// Copyright 2020 The Chromium Authors. All rights reserved.
function hasCtrlModifier(e){let hasModifier=e.ctrlKey;return hasModifier}function shouldIgnoreKeyEvents(){const activeElement=getDeepActiveElement();return activeElement.isContentEditable||activeElement.tagName==="INPUT"&&activeElement.type!=="radio"||activeElement.tagName==="TEXTAREA"}// Copyright 2015 The Chromium Authors. All rights reserved.
// Use of this source code is governed by a BSD-style license that can be
// found in the LICENSE file.
